...
const STATUS_LABELS = {
  ...
  not_interested: { label: 'Excluded', color: 'bg-rose-50 text-rose-600 border-rose-100 dark:bg-rose-950/20 dark:text-rose-500 dark:border-rose-900/20' },
};
    ...
    <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800/80 overflow-hidden text-left">
      {selected.length > 0 && (
        <div className="px-5 py-3 bg-blue-50/50 dark:bg-blue-950/20 border-b border-blue-100 dark:border-blue-900/30 flex items-center gap-4 flex-wrap">
          <span className="text-xs text-blue-800 dark:text-blue-400 font-extrabold uppercase tracking-wider">{selected.length} items selected</span>
          ...
          <div className="flex items-center gap-2">
            <select 
              ...
              className="text-[10px] font-black uppercase tracking-wider border border-slate-200 dark:border-slate-700 rounded-xl px-2.5 py-1.5 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 outline-none"
            >
          ...
          <button 
            onClick={bulkDelete} 
            className="text-[10px] text-rose-600 dark:text-rose-400 hover:text-rose-700 font-black uppercase tracking-wider flex items-center gap-1.5"
          >
            <Trash2 size={13} /> 
      ...
      {leads.length === 0 ? (
        <div className="p-16 text-center">
          <p className="text-slate-500 dark:text-slate-500 text-xs font-semibold uppercase tracking-wider">No matching prospects detected</p>
          <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-1 uppercase tracking-wider">Configure search settings or upload CSV to import.</p>
        </div>
      ) : (
          ...
          <table className="w-full text-xs">
            <thead className="border-b border-slate-50 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/60">
              <tr>
                ...
                <th className="px-5 py-3.5 text-left text-[9px] font-black text-slate-500 dark:text-slate-500 uppercase tracking-widest">Prospect Profile</th>
                <th className="px-5 py-3.5 text-left text-[9px] font-black text-slate-500 dark:text-slate-500 uppercase tracking-widest">Target Company</th>
                <th className="px-5 py-3.5 text-left text-[9px] font-black text-slate-500 dark:text-slate-500 uppercase tracking-widest">Prospect Fit index</th>
                <th className="px-5 py-3.5 text-left text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Pipeline State</th>
                <th className="px-5 py-3.5 text-left text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Last View Action</th>
                <th className="px-5 py-3.5 text-left text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">AI Sentiment</th>
                <th className="px-5 py-3.5 text-left text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest w-20">Actions</th>
              </tr>
            </thead>
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
              {leads.map((lead) => (
                ...
                >
                    ...
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-slate-50 dark:bg-slate-800 shrink-0 overflow-hidden border border-slate-100 dark:border-slate-700">
                        {lead.profile_photo_url ? (
                          ...
                          <div className="w-full h-full flex items-center justify-center text-slate-400 dark:text-slate-500 text-[9px] font-black uppercase">
                            {lead.full_name?.charAt(0) || 'U'}
                          </div>
                        )}
                      </div>
                      <div className="min-w-0">
                        <p className="font-extrabold text-slate-800 dark:text-slate-100 leading-tight">{lead.full_name}</p>
                  ...
                  <td className="px-5 py-4 text-slate-600 dark:text-slate-300 font-semibold">{lead.company || '—'}</td>
                  ...
                  <td className="px-5 py-4">
                    {lead.fit_score > 0 ? (
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="font-extrabold text-slate-700 dark:text-slate-200">{lead.fit_score}%</span>
                      </div>
                    ) : (
                      <span className="text-slate-300 dark:text-slate-600 text-[10px] italic font-semibold">Queueing...</span>
                    )}
                  </td>
                  ...
                  <td className="px-5 py-4">
                    {lead.reply_received ? (
                      <span className="flex items-center gap-1 text-emerald-600 dark:text-emerald-400 text-[10px] font-black uppercase tracking-wider">
                    ...
                    <div className="flex items-center gap-1.5" onClick={(e) => e.stopPropagation()}>
                      {lead.linkedin_url && (
                        <a 
                          ...
                          className="p-1.5 text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-lg transition-colors"
                        >
