...
process.env.JWT_SECRET = 'test_jwt_secret_key_2026_super_secure_long_secret';
...
require.cache[require.resolve('../services/unipile')] = {
  id: require.resolve('../services/unipile'),
  loaded: true,
  exports: mockUnipile
};
require.cache[require.resolve('../services/nvidia')] = {
  id: require.resolve('../services/nvidia'),
  loaded: true,
  exports: mockNvidia
};
...
const { runSendConnections, runFlowExecution } = require('../services/automation');
...
async function executeJourney() {
  ...
  // Cleanup database of any test records globally
  db.prepare('DELETE FROM users WHERE email = ?').run(testEmail);
  db.prepare("DELETE FROM leads WHERE linkedin_member_id = 'member_journey_candidate_abc' OR linkedin_url = 'https://www.linkedin.com/in/journey-candidate'").run();
  ...
  try {
    ...
    // -------------------------------------------------------------------------
    // Set campaign status to active via the correct dedicated status route
    await authClient.put(`/campaigns/${campaignId}/status`, {
      status: 'active'
    });
    
    // Set warmup_week to 4 and ensure the account is active/active and clear cooldown timers
    db.prepare("UPDATE accounts SET warmup_week = 4, is_active = 1, status = 'active', next_action_at = NULL, last_action_at = NULL WHERE id = ?").run(accountId);
    ...
    const diagAcc = db.prepare('SELECT * FROM accounts WHERE id = ?').get(accountId);
    const diagCamp = db.prepare('SELECT * FROM campaigns WHERE id = ?').get(campaignId);
    const diagLead = db.prepare('SELECT * FROM leads WHERE id = ?').get(leadId);
    const diagCA = db.prepare('SELECT * FROM campaign_accounts WHERE campaign_id = ?').all(campaignId);
    console.log('  --- DIAGNOSTICS ---');
    console.log('  Account:', JSON.stringify(diagAcc));
    console.log('  Campaign:', JSON.stringify(diagCamp));
    console.log('  Campaign Accounts Mapping:', JSON.stringify(diagCA));
    console.log('  Lead:', JSON.stringify(diagLead));

    const safety = require('../services/safety');
    const safetyCheck = safety.canSendConnection(diagAcc, diagCamp, db);
    console.log('  canSendConnection result:', JSON.stringify(safetyCheck));

    console.log('  Running Playbook Sequencer (Invite Exec)...');
    await runFlowExecution();

    const leadAfterConnection = db.prepare('SELECT status, account_id_used, connection_sent_at, flow_executions FROM leads WHERE id = ?').get(leadId);
    const execsFirst = JSON.parse(leadAfterConnection.flow_executions || '[]');
    const isInviteExecuted = execsFirst.some(e => e.node_id === 'invite-1');
    logTestResult(
      'Outbox Invite Executed via Sequencer',
      isInviteExecuted && leadAfterConnection.status === 'connection_sent' && leadAfterConnection.account_id_used === accountId,
      `Status: ${leadAfterConnection.status}, Account ID: ${leadAfterConnection.account_id_used}, Flow Execs: ${leadAfterConnection.flow_executions}`
    );
    ...
    // Clear cooldown of the account so next steps are not blocked
    db.prepare("UPDATE accounts SET next_action_at = NULL, last_action_at = NULL WHERE id = ?").run(accountId);
    await runFlowExecution();

    const leadAfterDelayCheck = db.prepare('SELECT status, flow_executions FROM leads WHERE id = ?').get(leadId);
    const execsAfterDelayCheck = JSON.parse(leadAfterDelayCheck.flow_executions || '[]');
    // Delay node should not resolve yet since accepted_at is brand new (requires 1 day delay)
    const hasDelayNodeExecuted = execsAfterDelayCheck.some(e => e.node_id === 'delay-1');
    logTestResult('Delay Node Correctly Blocks Execution', !hasDelayNodeExecuted, `Executed nodes: ${execsAfterDelayCheck.map(e => e.node_id).join(', ')}`);
    ...
    const updatedExecs = execsAfterDelayCheck.map(e => ({ ...e, executed_at: twoDaysAgo }));
